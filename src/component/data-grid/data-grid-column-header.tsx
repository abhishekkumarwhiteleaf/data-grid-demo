...
interface DataGridColumnHeaderProps<TData extends object, TValue>
  ...
  visibility?: boolean;
  /**
   * Custom content for the "Columns" submenu. When provided, it replaces the
   * default per-column visibility list and the submenu is shown regardless of
   * the `columnsVisibility`/`visibility` flags. Used by module tables to drive
   * column visibility from the layout.
   */
...
function DataGridColumnHeaderInner<TData extends object, TValue>({
  ...
  const headerButtonClassName = cn(
    "text-secondary-foreground/80 hover:bg-secondary inline-flex items-center justify-center rounded-full data-[state=open]:bg-secondary hover:text-foreground data-[state=open]:text-foreground px-2 font-normal h-6",
    className,
  );
  ...
  const showColumnsSection =
    !!columnsMenu || !!(props.tableLayout?.columnsVisibility && visibility);
