...
/**
 * 8px air between thead and tbody. Card and zebra layouts use it so the
 * first row does not sit flush under the header. Spreadsheet grids already
 * share a gridline there (`cellBorder`), and default `rowBorder` already
 * draws a divider — either way the gap would be a hole under the header.
 */
function shouldRenderDataGridTableRowSpacer(tableLayout?: {
  stripped?: boolean;
  rowBorder?: boolean;
  cellBorder?: boolean;
}) {
  if (tableLayout?.cellBorder) return false;
  return Boolean(tableLayout?.stripped || !tableLayout?.rowBorder);
}
...
function getDataGridRowExpandedContent<TData extends object>(
  table: DataGridTableInstance<TData>,
  props: { expandedContent?: (row: TData) => ReactNode },
) {
  return (
    props.expandedContent ??
    table.getAllColumns().find((column) => column.columnDef.meta?.expandedContent)?.columnDef.meta
      ?.expandedContent
  );
}

function canShowDataGridRowExpand<TData extends object>(
  row: Row<DataGridFeatures, TData>,
  table: DataGridTableInstance<TData>,
  props: { expandedContent?: (row: TData) => ReactNode },
) {
  if (!row.getCanExpand()) return false;
  // Tree rows expand into child rows of the same shape.
  if (row.subRows?.length) return true;
  // Detail rows expand into a full-width panel, not subRows.
  return !!getDataGridRowExpandedContent(table, props);
}
...
function DataGridTableBodyRowExpandded<TData extends object>({
  ...
  const { props, table } = useDataGrid<TData>();
  const expandedContent = getDataGridRowExpandedContent(table, props);

  // Tree and grouped rows share row.getIsExpanded() with detail expansion.
  // Without a detail renderer there is nothing to render, and an empty <tr>
  ...
  return (
    <tr
      data-slot="data-grid-table-row-expanded"
...
/**
 * Detail-expansion panel that subscribes to the expanded atom so React
 * Compiler (and the virtual row memo) cannot skip the panel after a toggle.
 */
function DataGridTableExpandedPanel<TData extends object>({
  row,
}: {
  row: Row<DataGridFeatures, TData>;
}) {
  const { table } = useDataGrid<TData>();
  const expandedAtom = table.atoms.expanded;

  const renderPanel = () =>
    row.getIsExpanded() ? <DataGridTableBodyRowExpandded row={row} /> : null;

  if (!expandedAtom) return renderPanel();

  return <Subscribe source={expandedAtom}>{renderPanel}</Subscribe>;
}
          ...
          <span
            ...
            className={cn(
              ...
              props.tableLayout?.cellFillHandleVariant === "ring"
                ? "border-primary bg-background -end-[5px] -bottom-[5px] size-[11px] border-2 in-[td:has(+[data-slot=data-grid-table-fill-body-cell]+td[data-pinned])]:end-0 in-[td:has(+[data-slot=data-grid-table-fill-body-cell]:last-child)]:end-0 in-[td:has(+td[data-pinned])]:end-0 in-[td:last-child]:end-0 in-[tr:last-child]:bottom-0"
                : props.tableLayout?.cellFillHandleVariant === "square"
                  ...
                  : "bg-primary border-background end-0 bottom-0 size-[9px] border",
...
function DataGridTableRenderedRow<TData extends object>({
  ...
  return (
    <Fragment>
      ...
      <DataGridTableExpandedPanel row={row} />
    </Fragment>
  );
}
...
function DataGridTableRowExpand<TData extends object>({
  ...
  const { i18n, props, table } = useDataGrid<TData>();
  const expandedAtom = table.atoms.expanded;
  const controlSize = props.tableLayout?.dense ? "size-6" : "size-7";

  const renderToggle = () => {
    const isExpanded = row.getIsExpanded();
    const canExpand = canShowDataGridRowExpand(row, table, props);
  ...
  // Same compiler trap as DataGridTableRowSelect: this lives in a cell
  // template with a stable row, so getIsExpanded() is invisible unless we
  // subscribe to the expanded atom.
  if (!expandedAtom) return renderToggle();

  return <Subscribe source={expandedAtom}>{renderToggle}</Subscribe>;
}

const DATA_GRID_SKELETON_ROW_CAP = 16;

function getDataGridSkeletonRowCount(pageSize?: number) {
  const size = pageSize && pageSize > 0 ? pageSize : 12;
  return Math.min(size, DATA_GRID_SKELETON_ROW_CAP);
}

function DataGridTableLoadingRows<TData extends object>({
  table,
}: {
  table: DataGridTableInstance<TData>;
}) {
  ...
  const rightVisibleColumns = table.getEndVisibleLeafColumns();
  const hasRightPinnedColumns = hasDataGridTableRightPinnedColumns(table);
  const skeletonCount = getDataGridSkeletonRowCount(pagination?.pageSize);
...
function DataGridTableBodyRows<TData extends object>({
  ...
  if (isLoading && props.loadingMode === "skeleton") {
    return <DataGridTableLoadingRows table={table} />;
  }
  ...
  return (
    <DataGridTableViewport>
      <DataGridTableBase>
        ...
        {renderHeader && shouldRenderDataGridTableRowSpacer(props.tableLayout) && (
