...
} from "@/components/data-grid/data-grid-i18n";
import {
...
export interface DataGridProps<TFeatures extends TableFeatures, TData extends object> {
  ...
  appendRow?: ReactNode;
  /**
   * Full-width panel under an expanded row (nested editors, sub-tables).
   * Tree expansion still uses `getSubRows`; this is the detail-row path.
   * Also accepted on a column's `meta.expandedContent`.
   */
  expandedContent?: (row: TData) => ReactNode;
  /**
...
function DataGridContainer({
  ...
  return (
    <div
      ...
      className={cn("relative w-full overflow-hidden", className, border && "border-border border")}
    >
      {children}
    </div>
  );
...
/**
 * Column defs typed against the grid's feature bundle. Prefer this over a
 * bare `ColumnDef<TData>` — v9 requires the feature generic first.
 */
export type DataGridColumnDef<TData extends object, TValue = unknown> = ColumnDef<
  DataGridFeatures,
  TData,
  TValue
>;
 ...
 * go through this so column defs, row models and selection stay in lockstep.
 */
export function useDataGridTable<TData extends object>(
