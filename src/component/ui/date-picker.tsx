...
type SharedPickerProps = {
  ...
  triggerOnKeyDown?: React.KeyboardEventHandler<HTMLButtonElement>
}
...
function DatePickerTrigger({
  ...
  return (
    <div className="rdp-input_control relative flex h-8 min-h-8 w-full items-center  " data-size={size} data-disabled={disabled || undefined}>
      <CalendarIcon className="rdp-input_icon pointer-events-none absolute left-2 top-1/2 size-4 -translate-y-1/2" aria-hidden="true" />
      <Input
        aria-hidden="true"
        className={cn("rdp-input_surface !box-border !h-8 !min-h-8 !w-full !pl-0 !pr-2 !text-sm !leading-5 ", triggerClassName)}
        disabled={disabled}
